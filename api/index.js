...
.btn{width:100%;padding:14px;font-size:16px;font-weight:600;background:#e8636b;color:#fff;border:none;border-radius:10px;cursor:pointer}
.btn:active{opacity:.85}
.btn:disabled{background:#ccc}
.result{background:#fff;border-radius:10px;padding:16px;line-height:1.8;font-size:15px;white-space:pre-wrap;word-break:break-word}
...
<textarea id="input" placeholder="直接把微信聊天记录复制粘贴到这里..."></textarea>
<div class="hint">你的对话不会存储</div></div>
<div class="section"><button class="btn" id="btn" onclick="run()">开始分析</button></div>
<div id="out"></div></div>
<div class="footer">分析基于学术研究 AI生成仅供参考</div>
<script>
async function run(){
var t=document.getElementById("input").value.trim();
var b=document.getElementById("btn");
var o=document.getElementById("out");
if(t.length<10){o.innerHTML='<div class="error">对话太短，至少10个字</div>';return}
...
const PROMPT = "你是相亲策略顾问，基于心理学研究提供分析。每条判断必须带出处标签：[学术]来自已发表研究、[数据]来自调查统计、[推理]基于逻辑推断须写推理链条。没有出处不下判断，结尾写可信度总结。分析框架：逐句拆解→整体判断→回复建议→止损判断。知识库：Knapp关系阶段、社会渗透理论、Gottman情感竞标、相互依赖理论、《亲密关系》。回复像人话，短句。";
